...
int is_selected(composite_ration Object, double min_protein, double max_carb,
                size_t index) {
  if (Object.array[index].protein >= min_protein &&
      Object.array[index].carb <= max_carb) {
    return 1;
  }
  return 0;
}
...
char* alloc_memory_for_name(composite_ration Object, size_t index) {
  Object.array[index].name.name =
      (char*)calloc(Object.array[index].name.size, sizeof(char));
