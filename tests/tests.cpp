...
TEST(TestInitRation, isInit) {
    ...
    strncpy(Object2.array[2].name.name, "Coffee with milk and cream", Object2.array[2].name.size);
    ...
    for (size_t i = 0; i < Object2.len_array - 1; ++i) {
        EXPECT_TRUE(!strncmp(Object.array[i].name.name, Object2.array[i].name.name, Object2.array[i].name.size));
...
TEST(TestSelectBreakfast, isSelected) {
    ...
    expected_dish.names = (char**) calloc(Object.len_array, sizeof(char*));
    expected_dish.names[0] = (char* ) calloc(Object.array[0].name.size, sizeof(char));
    expected_dish.names[1] = (char* ) calloc(Object.array[2].name.size, sizeof(char));

    strncpy(expected_dish.names[0], "Omelet", Object.array[0].name.size);
    strncpy(expected_dish.names[1], "Coffee with milk and cream", Object.array[2].name.size);


    selected_array dish = select_breakfast(Object, Filter);
//    printf("0 - %s\n", expected_dish.names[0]);
//    printf("0 - %s\n", dish.names[0]);
//    printf("1 - %s\n", expected_dish.names[1]);
//    printf("1 - %s\n", dish.names[1]);

    EXPECT_TRUE(!strncmp(expected_dish.names[0], dish.names[0], Object.array[0].name.size));
    EXPECT_TRUE(!strncmp(expected_dish.names[1], dish.names[1], Object.array[2].name.size));
